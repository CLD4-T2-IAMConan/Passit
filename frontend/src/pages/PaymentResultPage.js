  ...
  useEffect(() => {
    ...
    const completePayment = async () => {
      try {
        ...
        setMessage(
          "백엔드 처리 오류 발생. (임시) 결제 완료 화면으로 전환되었습니다. 실제 결제는 실패했을 수 있습니다."
        );
        // ----------------------------------------------------
  ...
  if (status === "FAILED") {
    ...
    return (
      <div style={styles.container}>
        ...
        <button onClick={() => navigate("/payment/retry")} style={styles.failButton}>
          결제 재시도하기
        </button>
      </div>
    );
  ...
  if (status === "SUCCESS") {
    return (
      <div style={styles.container}>
        ...
        <button onClick={() => navigate("/")} style={styles.successButton}>
          메인 페이지로 돌아가기
        </button>
      </div>
    );
