...
const BuyerPaymentPage = () => {
    ...
    const fetchPaymentData = useCallback(async () => {
        ...
        try {
            ...
            // 🚨 [수정] 에러 메시지 처리 강화
            const errorMessage = err.response?.data?.error || err.response?.data || '정보를 불러오는 데 실패했습니다.';
    ...
    if (currentUser === undefined || loading) return <Box sx={{ p: 4, textAlign: 'center' }}><CircularProgress /></Box>;
    if (currentUser === null) return <Alert severity="error" sx={{ m: 4 }}>사용자 정보를 불러오지 못했거나 로그인되어 있지 않습니다.</Alert>;
